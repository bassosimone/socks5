...
// addrSpec is used to return the target addrSpec which
// may be specified as IPv4, IPv6, or a FQDN
type addrSpec struct {
	Address string
	Port    int
}

// A request is a request received by the [*Server].
type request struct {
	// Protocol version
	Version uint8
...
func (s *Server) handleRequest(ctx context.Context, req *request, cconn net.Conn) error {
...
func (s *Server) handleConnect(ctx context.Context, cconn net.Conn, req *request) error {
	...
	s.Logger.Info("handleConnect", slog.String("endpoint", endpoint))
	sconn, err := s.Dialer.DialContext(ctx, "tcp", endpoint)
	if err != nil {
	...
	// Make sure we close sconn when the context is canceled
	stop := context.AfterFunc(ctx, func() { sconn.Close() })
	defer stop()
